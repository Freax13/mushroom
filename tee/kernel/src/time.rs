use crate::{
    error::{Result, bail},
    exception::{DisableAllInterruptsGuard, InterruptGuard},
    spin::mutex::Mutex,
    user::{
...
pub fn advance_time(last_vcpu_guard: LastRunningVcpuGuard) -> Result<(), NoTimeoutScheduledError> {
    ...
    let guard = DisableAllInterruptsGuard::new();
...
struct TimerLists {
    ...
    left: UnsafeCell<Half>,
    right: UnsafeCell<Half>,
}
...
unsafe impl Sync for TimerLists {}
...
impl ManagementLock {
    ...
    fn release_lock(&mut self) {
        ...
        loop {
            ...
            if !state.processed() {
                ...
                let tick = state.tick();
                self.process(unsafe { &mut *half }, tick);
    ...
    fn process(&mut self, half: &mut Half, tick: Tick) {
        ...
        let mut cursor = self.rb_tree_left.front_mut();
        while cursor.get().is_some_and(|node| node.deadline <= tick) {
            cursor.remove().unwrap();
        }
    }
