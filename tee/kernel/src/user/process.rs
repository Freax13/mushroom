...
impl Process {
    ...
    pub fn mm_env_start(&self) -> VirtAddr {
        self.mm_env_start.load()
    }

    pub fn set_mm_env_start(&self, addr: VirtAddr) {
        self.mm_env_start.store(addr);
    }

    pub fn mm_env_end(&self) -> VirtAddr {
        self.mm_env_end.load()
    }
