...
use nix::{
    errno::Errno,
    libc::{c_ulong, ioctl, Ioctl},
    poll::{poll, PollFd, PollFlags, PollTimeout},
    sys::socket::{
        self, getpeername, getsockname, setsockopt, socket,
...
fn test_oob() -> Result<()> {
    ...
    // Sending OOB data resets the marker flag.
    ...
    poll(core::slice::from_mut(&mut fd), PollTimeout::ZERO);
    ...
    assert_eq!(socket::recv(fd2, &mut [], MsgFlags::MSG_DONTWAIT), Ok(0));
